...
void FixedUpdate () 
{
		transform.rotation = Quaternion.identity;						//disable the character rotation
		...
		if (Input.GetButtonDown("Fire1"))								//if Ctrl is in downstate
			Instantiate(Bullet, Bazooka.transform.position, transform.rotation); //then create a rocket at bazooka's coordinates
